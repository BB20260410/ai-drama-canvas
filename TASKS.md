...
- status: `阻塞`（本 Cloud；整计划 `部分完成`）。§1.3 by-id + 诊断懒展开 + lean 季集身份 + 身份只读 + 账本批读只读 + 历史 PASS 活库只读 + Linux 真投影 sqlite 接线 + 零 mock inspect 已落地。等待 owner：P7 owner / 安装版 T23 / 列表 GUI 探针
