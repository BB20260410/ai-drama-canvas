/**
 * S1E2-U24 父洞口回头·纹金亮一瞬 — create→bind only
 * 后续：npx tsx scripts/s1e2-mcp-only-runner.ts prepare --unit S1E2-U24
 */
...
const LOG = path.join(SCRATCH, "s1e2-u24-ordered-transcript.log");
const UNIT_ID = "S1E2-U24";
const SCRIPT_BODY =
  "S1E2-U24 父在洞口回头。纹金亮一瞬。石穴洞口外。";

const SURFACE: Record<string, string[]> = {
  "scene-shixue": ["石穴", "洞口", "洞口外"],
  "char-shuo": ["父"],
};
...
async function ensureUnit() {
  ...
  const scriptDoc = await createStudioScriptDocument(ROOT, { title: "S1E2-U24 洞口回头", expectedRevision: 0 });
  const revWrap = await appendStudioScriptRevision(ROOT, {
    documentId: scriptDoc.id, expectedRevision: 0, body: SCRIPT_BODY, source: "s1e2-u24", sourceVersion: "20260723",
  });
  const rev = revWrap.revision;
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U24 R-CINE father cave mouth gold flash",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id, expectedRevision: 0,
    body: "9:16 vertical THREE panels pure mineral mural fresco lapis gold. Silent unit zero dialogue text. G1 medium REAL: father black-and-white wolf at cave mouth turns head back, silhouette against blue-black night. G2 close-up: gold rune veins on father fur flash once then dim. G3 wide: cave mouth stone frame, residual gold spark fades into dark forest edge. NO humans faces NO text. Photoreal forbidden. Lock identities from control refs when present.",
    source: "s1e2-u24", sourceVersion: "20260724",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u24" }],
  });
  // 4+4+2.5+4.5=15
  // 5+5+5=15
  const panels = [
    {
      title: "G1 回头", visualAction: "父在洞口回头",
  ...
  const unitSnap = await createStudioProductionUnit(ROOT, {
    id: UNIT_ID, expectedRevision: 0, season: "S1", episode: "S1E2", sequence: 24,
    title: "场5·洞口回头", durationSeconds: 15, scriptRevisionId: rev.id, panels,
  });
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u24-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          role: `u24-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          expectedHeadRevision: tryRev, mentions, resolverVersion: `s1e2-u24-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(await recordStudioMentionDecision(ROOT, {
            receiptId: `u24-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
            proposalId: proposal.id, expectedAnalysisHeadRevision: analysis.revision, expectedDecisionHeadRevision: 0,
            action, selectedAssetId, presence: proposal.presence, role: proposal.role, reviewer: "s1e2-u24", note: "u22",
          }));
        }
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u24-cont-${panel.id}-${src.assetId}-${field}`, expectedHeadRevision: 0, scope,
            subjectId: src.assetId, field,
            state: {
              status: "resolved", value,
              provenance: [{
                kind: "s1e2-u24", reference: `${panel.id}/${src.assetId}/${field}`,
