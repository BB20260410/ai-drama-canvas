/**
 * S1E2-U15 场3 亮起又熄·刀＋星海补天 — create→bind only
 * 后续：npx tsx scripts/s1e2-ordered-runner.ts prepare --unit S1E2-U15
 *      → gen → ordered-runner commit
 * 半旁路；有序 formal PASS ≠ 产品完美契合
 */
...
const LOG = path.join(SCRATCH, "s1e2-u15-ordered-transcript.log");
const UNIT_ID = "S1E2-U15";
// 面词唯一：画卷/大地/战场/星海/天缺/天狗/先祖 分给 scene 与 char
const SCRIPT_BODY =
  "S1E2-U15 画卷流星纹亮到极处。天狗剪影伏倒贴进大地刻线。先祖额纹接着亮起踏前。战场星海补天天缺仍悬。";

const SURFACE: Record<string, string[]> = {
  "scene-shixue": ["画卷", "大地", "战场", "星海", "天缺", "刻线", "流星纹"],
  "char-shuo": ["天狗", "先祖"],
};
...
async function ensureUnit() {
  ...
  const scriptDoc = await createStudioScriptDocument(ROOT, { title: "S1E2-U15 亮起又熄·星海补天", expectedRevision: 0 });
  const revWrap = await appendStudioScriptRevision(ROOT, {
    documentId: scriptDoc.id, expectedRevision: 0, body: SCRIPT_BODY, source: "s1e2-u15", sourceVersion: "20260723",
  });
  const rev = revWrap.revision;
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U15 R-CINE blaze fall star-sea mend sky",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id, expectedRevision: 0,
    body: "9:16 vertical FOUR panels pure mineral mural fresco rock pigment gold leaf. G1 extreme close-up: one meteor-vein mark blazes to peak, scorching gold nearly swallows frame, light flame licks forehead fur as engraved lines. G2 medium: that tiangou silhouette, vein light dies, body slowly collapses into earth seal engraving lines, becomes part of the pattern, no blood no wound. G3 close-up: neighboring tiangou forehead vein lights next, side profile jaw set, steps forward without looking at fallen comrade. G4 extreme wide high-angle crane-up feel: whole battlefield, vein lights breathe on-off like star sea, red seal lines filling, camera rises until star-sea meets true milky way; thin pure-black sky-missing slit still hangs at top edge unclosed. NO humans faces NO text. Photoreal forbidden. White-headed black-body jade-wolf tiangou only.",
    source: "s1e2-u15", sourceVersion: "20260723",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u15" }],
  });
  // 2.5+2.5+2.5+7.5=15
  const panels = [
    {
      title: "G1 纹灼顶点", visualAction: "一道流星纹亮到极处灼金吞画",
    ...
    {
      title: "G2 伏倒入印", visualAction: "天狗剪影纹熄伏倒贴进大地刻线",
      shotComposition: "中景", filmingMethod: "呼吸感固定",
      startSeconds: 2.5, durationSeconds: 2.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene"), mk("char-shuo", "character")],
    },
    {
      title: "G3 接着亮", visualAction: "邻犬额纹亮起踏前不看同伴",
      shotComposition: "近景", filmingMethod: "呼吸感固定",
      startSeconds: 5, durationSeconds: 2.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene"), mk("char-shuo", "character")],
    },
    {
      title: "G4 星海补天", visualAction: "战场俯瞰升镜星海补天缺仍悬",
      shotComposition: "大远景俯瞰", filmingMethod: "升起",
      startSeconds: 7.5, durationSeconds: 7.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene")],
    },
  ];
  const unitSnap = await createStudioProductionUnit(ROOT, {
    id: UNIT_ID, expectedRevision: 0, season: "S1", episode: "S1E2", sequence: 15,
    title: "封印·亮起又熄·星海补天", durationSeconds: 15, scriptRevisionId: rev.id, panels,
  });
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u15-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          role: `u15-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          expectedHeadRevision: tryRev, mentions, resolverVersion: `s1e2-u15-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(await recordStudioMentionDecision(ROOT, {
            receiptId: `u15-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
            proposalId: proposal.id, expectedAnalysisHeadRevision: analysis.revision, expectedDecisionHeadRevision: 0,
            action, selectedAssetId, presence: proposal.presence, role: proposal.role, reviewer: "s1e2-u15", note: "u15",
          }));
        }
    ...
    const scope = {
      ...
      startMilliseconds: Math.round(panel.startSeconds * 1000), endMilliseconds: Math.round(panel.endMilliseconds ?? panel.endSeconds * 1000),
    };
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u15-cont-${panel.id}-${src.assetId}-${field}`, expectedHeadRevision: 0, scope,
            subjectId: src.assetId, field,
            state: {
              status: "resolved", value,
              provenance: [{
                kind: "s1e2-u15", reference: `${panel.id}/${src.assetId}/${field}`,
...
async function main() {
  mkdirSync(SCRATCH, { recursive: true });
  writeFileSync(LOG, "");
  log("=== S1E2-U15 create+bind (ordered-runner 后续 prepare) ===");
  ...
  log(`BIND_OK ${unitId} → next: ordered-runner prepare --unit ${unitId}`);
  console.log(JSON.stringify({ ok: true, unitId, next: "npx tsx scripts/s1e2-ordered-runner.ts prepare --unit S1E2-U15" }, null, 2));
}
