/**
 * S1E2-U19 灭血断封 — create→bind only
 */
...
const LOG = path.join(SCRATCH, "s1e2-u19-ordered-transcript.log");
const UNIT_ID = "S1E2-U19";
const SCRIPT_BODY =
  "S1E2-U19 画卷群山纹光递灭灭血。山谷谷口封纹中断断封。断口光丝流失。";

const SURFACE: Record<string, string[]> = {
  "scene-shixue": ["画卷", "群山", "山谷", "谷口", "封纹", "断口"],
};
...
async function ensureUnit() {
  ...
  const scriptDoc = await createStudioScriptDocument(ROOT, { title: "S1E2-U19 灭血断封", expectedRevision: 0 });
  const revWrap = await appendStudioScriptRevision(ROOT, {
    documentId: scriptDoc.id, expectedRevision: 0, body: SCRIPT_BODY, source: "s1e2-u19", sourceVersion: "20260723",
  });
  const rev = revWrap.revision;
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U19 R-CINE extinguish blood break seal",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id, expectedRevision: 0,
    body: "9:16 vertical THREE panels pure mineral mural fresco lapis gold. NO humans faces NO text NO photoreal. G1 wide pan: mountain scroll vein lights extinguish one after another as pan moves, gold dots swallowed by blue-black. G2 medium: valley mouth seal engraving same family as cave seal cracks from center, two halves darken, mouth open. G3 extreme close-up: seal fracture, last gold light threads leak out like hourglass sand until grey dead lines. NO monster bodies.",
    source: "s1e2-u19", sourceVersion: "20260723",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u19" }],
  });
  // 5+5.5+4.5=15
  const panels = [
    {
      title: "G1 灭血", visualAction: "群山纹光递灭一盏盏灭",
    ...
    {
      title: "G2 断封", visualAction: "谷口封纹中断两截暗去",
      shotComposition: "中景", filmingMethod: "呼吸感固定",
      startSeconds: 5, durationSeconds: 5.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene")],
    },
    {
      title: "G3 光丝流失", visualAction: "断口光丝漏尽成灰",
  ...
  await createStudioProductionUnit(ROOT, {
    id: UNIT_ID, expectedRevision: 0, season: "S1", episode: "S1E2", sequence: 19,
    title: "封印·灭血断封", durationSeconds: 15, scriptRevisionId: rev.id, panels,
  });
  log(`CREATE ${UNIT_ID}`);
  return UNIT_ID;
}
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u19-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          role: `u19-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          expectedHeadRevision: tryRev, mentions, resolverVersion: `s1e2-u19-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(await recordStudioMentionDecision(ROOT, {
            receiptId: `u19-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
            proposalId: proposal.id, expectedAnalysisHeadRevision: analysis.revision, expectedDecisionHeadRevision: 0,
            action, selectedAssetId, presence: proposal.presence, role: proposal.role, reviewer: "s1e2-u19", note: "u19",
          }));
        }
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u19-cont-${panel.id}-${src.assetId}-${field}`, expectedHeadRevision: 0, scope,
            subjectId: src.assetId, field,
            state: {
              status: "resolved", value,
              provenance: [{
                kind: "s1e2-u19", reference: `${panel.id}/${src.assetId}/${field}`,
