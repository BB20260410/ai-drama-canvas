...
const blockingRunId = "codex-ug-run-mrwdwtyh";
...
// Fail partial in-flight run from aborted prepare so we can re-dispatch cleanly
try {
  await failStudioGenerationRun(root, {
    generationRunId: blockingRunId,
    reason: "aborted-prepare-after-brief-schema-mismatch; reopen codex connect path",
    expectedRevision: 0,
  });
  console.error("failed blocking run", blockingRunId);
} catch (e) {
  // try via execute_command
  try {
    await executeIdempotentCommand(root, {
      requestId: `fail-run-${Date.now()}`,
      idempotencyKey: `fail-run-key-${blockingRunId}`,
      request: {
    ...
    console.error("failed via execute_command");
  } catch (e2) {
    console.error("fail run error", e.message, e2.message);
  }
}
...
const out = {
  ...
  projectContextToken: context.projectContextToken,
  renderedPrompt: pack.request?.modelPayload?.renderedPrompt || null,
};
