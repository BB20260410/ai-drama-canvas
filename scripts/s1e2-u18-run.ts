/**
 * S1E2-U18 群山散尽+红雾讳名 — create→bind only
 * 后续：mcp-only prepare → gen → commit → 单元复盘闸
 */
...
const LOG = path.join(SCRATCH, "s1e2-u18-ordered-transcript.log");
const UNIT_ID = "S1E2-U18";
const SCRIPT_BODY =
  "S1E2-U18 画卷群山连绵纹光散山脊。山谷各入谷口合拢蜀山远。铁锈红雾逆风掠脊。红雾过处纹光灭。";

const SURFACE: Record<string, string[]> = {
  "scene-shixue": ["画卷", "群山", "山谷", "蜀山", "山脊", "谷口"],
};
...
async function ensureUnit() {
  ...
  const scriptDoc = await createStudioScriptDocument(ROOT, { title: "S1E2-U18 群山·红雾讳名", expectedRevision: 0 });
  const revWrap = await appendStudioScriptRevision(ROOT, {
    documentId: scriptDoc.id, expectedRevision: 0, body: SCRIPT_BODY, source: "s1e2-u18", sourceVersion: "20260723",
  });
  const rev = revWrap.revision;
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U18 R-CINE mountains red-mist taboo-name",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id, expectedRevision: 0,
    body: "9:16 vertical FOUR panels pure mineral mural fresco lapis blue gold. NO monster body NO claws NO teeth NO wings. G1 extreme wide pan: continuous mountains, sparse meteor-vein light dots on ridges, cannot see each other. G2 wide: vein lights enter different valleys, valley mouths close behind, brightest light walks far right distant highest mountain. G3 wide: ONE thin rust-red mist stroke against wind direction across ridge, wind engraving lines reverse where mist passes; no creature form. G4 medium: mist passes ridge, one vein light extinguishes instantly no struggle, mist continues off-frame. NO humans faces NO text. Photoreal forbidden.",
    source: "s1e2-u18", sourceVersion: "20260723",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u18" }],
  });
  // 4+2.5+5.5+3=15
  const panels = [
    {
      title: "G1 群山散", visualAction: "群山连绵纹光星点散山脊",
      shotComposition: "大远景", filmingMethod: "水平摇",
      startSeconds: 0, durationSeconds: 4, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene")],
    },
    {
      title: "G2 各入谷", visualAction: "纹光入谷谷口合拢最亮向蜀山",
      shotComposition: "远景", filmingMethod: "呼吸感固定",
      startSeconds: 4, durationSeconds: 2.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene")],
    },
    {
      title: "G3 红雾一丝", visualAction: "铁锈红雾逆风掠山脊无本体",
      shotComposition: "远景", filmingMethod: "呼吸感固定",
      startSeconds: 6.5, durationSeconds: 5.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene")],
    },
    {
      title: "G4 过处即灭", visualAction: "红雾过山脊纹光灭",
  ...
  const unitSnap = await createStudioProductionUnit(ROOT, {
    id: UNIT_ID, expectedRevision: 0, season: "S1", episode: "S1E2", sequence: 18,
    title: "封印·群山·红雾讳名", durationSeconds: 15, scriptRevisionId: rev.id, panels,
  });
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u18-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          role: `u18-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          expectedHeadRevision: tryRev, mentions, resolverVersion: `s1e2-u18-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(await recordStudioMentionDecision(ROOT, {
            receiptId: `u18-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
            proposalId: proposal.id, expectedAnalysisHeadRevision: analysis.revision, expectedDecisionHeadRevision: 0,
            action, selectedAssetId, presence: proposal.presence, role: proposal.role, reviewer: "s1e2-u18", note: "u18",
          }));
        }
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u18-cont-${panel.id}-${src.assetId}-${field}`, expectedHeadRevision: 0, scope,
            subjectId: src.assetId, field,
            state: {
              status: "resolved", value,
              provenance: [{
                kind: "s1e2-u18", reference: `${panel.id}/${src.assetId}/${field}`,
