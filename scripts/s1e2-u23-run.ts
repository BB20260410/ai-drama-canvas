/**
 * S1E2-U23 母舔额不答·崽不再追问 — create→bind only
 * 后续：npx tsx scripts/s1e2-mcp-only-runner.ts prepare --unit S1E2-U23
 */
...
const LOG = path.join(SCRATCH, "s1e2-u23-ordered-transcript.log");
const UNIT_ID = "S1E2-U23";
const SCRIPT_BODY =
  "S1E2-U23 母舔崽额不答。崽耳尖落不再追问。石穴洞内静。";
...
async function ensureUnit() {
  ...
  const scriptDoc = await createStudioScriptDocument(ROOT, { title: "S1E2-U23 舔额不答", expectedRevision: 0 });
  const revWrap = await appendStudioScriptRevision(ROOT, {
    documentId: scriptDoc.id, expectedRevision: 0, body: SCRIPT_BODY, source: "s1e2-u23", sourceVersion: "20260723",
  });
  const rev = revWrap.revision;
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U23 R-CINE mother lick forehead silence",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id, expectedRevision: 0,
    body: "9:16 vertical FOUR panels pure mineral mural fresco lapis gold. Silent unit zero dialogue text. G1 medium REAL cave: mother wolf gently licks cub forehead, tender, no answer words. G2 close-up: mother tongue on cub brow, cub eyes half-close. G3 medium: cub ear tips lower, stops chasing question, leans into mother. G4 wide: quiet stone cave, residual gold dust, absolute silence. NO humans faces NO text. Photoreal forbidden. Lock identities from control refs when present.",
    source: "s1e2-u23", sourceVersion: "20260724",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u23" }],
  });
  // 4+4+2.5+4.5=15
  // 4+4+3.5+3.5=15
  const panels = [
    {
      title: "G1 舔额", visualAction: "母舔崽额不答",
    ...
    {
      title: "G2 额温", visualAction: "母舌过崽眉心",
      shotComposition: "近景", filmingMethod: "微推",
      startSeconds: 4, durationSeconds: 4, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene"), mk("char-dudu", "character"), mk("char-su", "character")],
    },
    {
      title: "G3 不再追问", visualAction: "崽耳尖落不再追问",
      shotComposition: "中近景", filmingMethod: "呼吸感固定",
      startSeconds: 8, durationSeconds: 3.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene"), mk("char-dudu", "character"), mk("char-su", "character")],
    },
    {
      title: "G4 洞静", visualAction: "石穴洞内静",
  ...
  const unitSnap = await createStudioProductionUnit(ROOT, {
    id: UNIT_ID, expectedRevision: 0, season: "S1", episode: "S1E2", sequence: 23,
    title: "场5·舔额不答", durationSeconds: 15, scriptRevisionId: rev.id, panels,
  });
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u23-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          role: `u23-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          expectedHeadRevision: tryRev, mentions, resolverVersion: `s1e2-u23-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(await recordStudioMentionDecision(ROOT, {
            receiptId: `u23-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
            proposalId: proposal.id, expectedAnalysisHeadRevision: analysis.revision, expectedDecisionHeadRevision: 0,
            action, selectedAssetId, presence: proposal.presence, role: proposal.role, reviewer: "s1e2-u23", note: "u22",
          }));
        }
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u23-cont-${panel.id}-${src.assetId}-${field}`, expectedHeadRevision: 0, scope,
            subjectId: src.assetId, field,
            state: {
              status: "resolved", value,
              provenance: [{
                kind: "s1e2-u23", reference: `${panel.id}/${src.assetId}/${field}`,
