/**
 * S1E2-U17 战场死寂+打断④ — create→bind only
 * 后续：npx tsx scripts/s1e2-mcp-only-runner.ts prepare --unit S1E2-U17
 */
...
const LOG = path.join(SCRATCH, "s1e2-u17-ordered-transcript.log");
const UNIT_ID = "S1E2-U17";
const SCRIPT_BODY =
  "S1E2-U17 画卷战场空镜烟尘沉降巨印完整。俯瞰封印巨纹脉搏微光。洞内崽耳尖伏下又立回一半。母怀里。幸存剪影立印上哀悼。";

const SURFACE: Record<string, string[]> = {
  "scene-shixue": ["画卷", "战场", "巨印", "封印", "洞内", "印上"],
  "char-dudu": ["崽"],
  "char-su": ["母"],
};
...
async function ensureUnit() {
  ...
  const scriptDoc = await createStudioScriptDocument(ROOT, { title: "S1E2-U17 死寂·耳尖伏下", expectedRevision: 0 });
  const revWrap = await appendStudioScriptRevision(ROOT, {
    documentId: scriptDoc.id, expectedRevision: 0, body: SCRIPT_BODY, source: "s1e2-u17", sourceVersion: "20260723",
  });
  const rev = revWrap.revision;
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U17 R-CINE silence seal pulse ear-fold",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id, expectedRevision: 0,
    body: "9:16 vertical FOUR panels pure mineral mural fresco. Silent unit zero dialogue. G1 extreme wide mural battlefield empty smoke dust settling into seal lines, giant seal complete dark, no vein lights. G2 wide high-angle mural: complete seal engraving fills earth, micro-gold pulse light like heartbeat. G3 close-up REAL cave: white-headed black pup cub side face and ear tips in mother wolf arms, ear tips gently fold then half-rise, mural glow faint, zero dialogue interrupt. G4 medium mural: few surviving tiangou silhouettes stand motionless on seal lines mourning, wind-line engravings. NO humans faces NO text. Photoreal forbidden. Lock identities from control refs when present.",
    source: "s1e2-u17", sourceVersion: "20260723",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u17" }],
  });
  // 4+4+2.5+4.5=15
  const panels = [
    {
      title: "G1 战场死寂", visualAction: "空镜烟尘沉降巨印完整无纹光",
    ...
    {
      title: "G2 印脉", visualAction: "俯瞰巨印微光如大地脉搏",
      shotComposition: "远景俯瞰", filmingMethod: "微推",
      startSeconds: 4, durationSeconds: 4, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene")],
    },
    {
      title: "G3 耳尖伏下", visualAction: "崽耳尖伏下又立回一半母怀里",
      shotComposition: "近景", filmingMethod: "呼吸感固定",
      startSeconds: 8, durationSeconds: 2.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene"), mk("char-dudu", "character"), mk("char-su", "character")],
    },
    {
      title: "G4 立影哀悼", visualAction: "幸存剪影立印上不动",
      shotComposition: "中景", filmingMethod: "呼吸感固定",
      startSeconds: 10.5, durationSeconds: 4.5, promptRevisionId: promptRev.id, sourceSpans: [span],
      assets: [mk("scene-shixue", "scene")],
    },
  ];
  const unitSnap = await createStudioProductionUnit(ROOT, {
    id: UNIT_ID, expectedRevision: 0, season: "S1", episode: "S1E2", sequence: 17,
    title: "封印·死寂·耳尖", durationSeconds: 15, scriptRevisionId: rev.id, panels,
  });
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u17-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          role: `u17-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          expectedHeadRevision: tryRev, mentions, resolverVersion: `s1e2-u17-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(await recordStudioMentionDecision(ROOT, {
            receiptId: `u17-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
            proposalId: proposal.id, expectedAnalysisHeadRevision: analysis.revision, expectedDecisionHeadRevision: 0,
            action, selectedAssetId, presence: proposal.presence, role: proposal.role, reviewer: "s1e2-u17", note: "u17",
          }));
        }
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u17-cont-${panel.id}-${src.assetId}-${field}`, expectedHeadRevision: 0, scope,
            subjectId: src.assetId, field,
            state: {
              status: "resolved", value,
              provenance: [{
                kind: "s1e2-u17", reference: `${panel.id}/${src.assetId}/${field}`,
