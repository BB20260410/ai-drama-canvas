/**
 * S1E2-U20 画卷收束·一脉单传 — create→bind only
 */
...
const LOG = path.join(SCRATCH, "s1e2-u20-ordered-transcript.log");
const UNIT_ID = "S1E2-U20";
const SCRIPT_BODY =
  "S1E2-U20 画卷收束蜀山一点。群山墨线向中心收缩。一脉单传金线独亮。";

const SURFACE: Record<string, string[]> = {
  "scene-shixue": ["画卷", "蜀山", "群山", "金线", "一脉"],
};
...
async function ensureUnit() {
  ...
  const scriptDoc = await createStudioScriptDocument(ROOT, { title: "S1E2-U20 一脉单传", expectedRevision: 0 });
  const revWrap = await appendStudioScriptRevision(ROOT, {
    documentId: scriptDoc.id, expectedRevision: 0, body: SCRIPT_BODY, source: "s1e2-u20", sourceVersion: "20260724",
  });
  const rev = revWrap.revision;
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U20 R-CINE scroll converge single lineage",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id, expectedRevision: 0,
    body: "9:16 vertical THREE panels pure mineral mural fresco lapis gold. NO humans faces NO text NO photoreal. G1 wide: vast mountain scroll ink-lines slowly converge toward one distant peak, gold dust drifts inward. G2 medium: mountain range collapses into a single Shu peak silhouette, blue-black field, one gold vein remains. G3 extreme close-up: that single gold lineage vein alone burns bright on cold stone, all other lights dead grey. NO monster bodies.",
    source: "s1e2-u20", sourceVersion: "20260724",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u20" }],
  });
  // 5+5+5=15
  const panels = [
    {
      title: "G1 收束", visualAction: "画卷群山墨线向中心收缩",
  ...
  await createStudioProductionUnit(ROOT, {
    id: UNIT_ID, expectedRevision: 0, season: "S1", episode: "S1E2", sequence: 20,
    title: "封印·一脉单传", durationSeconds: 15, scriptRevisionId: rev.id, panels,
  });
  log(`CREATE ${UNIT_ID}`);
  return UNIT_ID;
}
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u20-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          role: `u20-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          expectedHeadRevision: tryRev, mentions, resolverVersion: `s1e2-u20-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(await recordStudioMentionDecision(ROOT, {
            receiptId: `u20-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
            proposalId: proposal.id, expectedAnalysisHeadRevision: analysis.revision, expectedDecisionHeadRevision: 0,
            action, selectedAssetId, presence: proposal.presence, role: proposal.role, reviewer: "s1e2-u20", note: "u20",
          }));
        }
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u20-cont-${panel.id}-${src.assetId}-${field}`, expectedHeadRevision: 0, scope,
            subjectId: src.assetId, field,
            state: {
              status: "resolved", value,
              provenance: [{
                kind: "s1e2-u20", reference: `${panel.id}/${src.assetId}/${field}`,
