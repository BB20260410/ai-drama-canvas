...
const registryPath = path.resolve(process.argv[3] || "/tmp/ai-canvas-artlist-preflight-blocked-ui-registry-20260715.json");
const evidencePath = path.resolve(process.argv[4] || path.join(workspace, "docs/evidence/artlist-preflight-blocked-ui-smoke-20260715.json"));
const screenshotPath = path.resolve(process.argv[5] || path.join(workspace, "docs/evidence/artlist-preflight-blocked-ui-smoke-20260715.png"));
...
try {
  ...
  await row.locator(".preflight-blocked").waitFor();
  ...
  if (!apiState.job || apiState.jobCount !== 6 || apiState.job.status !== "waiting_external" || apiState.job.browserState !== "preflight_blocked") throw new Error(`P01 API 状态不正确：${JSON.stringify(apiState)}`);
  if (apiState.job.browserCheckpoint?.revision !== 2 || apiState.job.browserCheckpoint.stage !== "preflight_blocked") throw new Error(`P01 检查点不是 R2 / preflight_blocked：${JSON.stringify(apiState.job.browserCheckpoint)}`);
  const blockers = apiState.job.browserCheckpoint.preflightEvidence?.blockers ?? [];
  if (blockers.join(",") !== "insufficient_credits,generation_mode_mismatch" || apiState.job.browserCheckpoint.preflightEvidence?.observedGeneration?.generateEnabled !== false) throw new Error(`P01 阻塞证据不完整：${JSON.stringify(apiState.job.browserCheckpoint.preflightEvidence)}`);
  ...
  const rendered = await row.evaluate((element) => ({
    ...
    blocked: element.querySelector(".preflight-blocked")?.textContent?.replace(/\s+/gu, " ").trim(),
  ...
  if (rendered.status !== "预检阻塞" || !rendered.checkpoint?.includes("网页 preflight_blocked · R2") || rendered.blocked !== "点击前已停止 · 额度不足 / 生成参数不匹配") throw new Error(`P01 预检阻塞 UI 文本不完整：${JSON.stringify(rendered)}`);
  if (rendered.submitIntentCount || rendered.uploadSlotCount || rendered.pageOverflowX || rendered.pageOverflowY || pageErrors.length) throw new Error(`P01 UI 误显示副作用或发生溢出：${JSON.stringify({ rendered, pageErrors })}`);
  ...
  const report = {
    schemaVersion: 1,
    kind: "artlist-preflight-blocked-ui-smoke",
