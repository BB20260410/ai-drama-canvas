/**
 * S1E2-U08 画卷：白额第一道流星纹 — create→bind→freeze→dispatch→prepare
 */
...
const LOG = path.join(SCRATCH, "s1e2-u08-ordered-transcript.log");
const STATE = path.join(SCRATCH, "s1e2-u08-ordered-state.json");
const UNIT_ID = "S1E2-U08";
const SCRIPT_BODY =
  "S1E2-U08 画卷太古。金光缕点先祖白额。金银交缠第一道流星纹成形。父伏姿立约。新纹定亮。";
...
async function resolveControlLocalPaths(packId: string) {
  ...
  const refs =
    (pack as any).request?.controlReferences
    ?? (pack as any).controlReferences
    ?? [];
...
async function ensureUnit() {
  ...
  const promptDoc = await createStudioPromptDocument(ROOT, {
    title: "S1E2-U08 R-CINE mural first mark",
    expectedRevision: 0,
  });
  const promptWrap = await appendStudioPromptRevision(ROOT, {
    documentId: promptDoc.id,
    expectedRevision: 0,
    body: "9:16 vertical unit-grid FOUR panels. Ancient mineral mural: ochre, gold-leaf, silver-thread, lapis blue, engraved lines, crackle. G1 medium-close: gold light filament from pillar points to ancestor white forehead, he bows slightly. G2 extreme close forehead: gold+silver threads twist into first vertical meteor mark. G3 medium: ancestor prostrates, mark glowing against star river. G4 close: mark settles warm glow. Jade-wolf white head black body from char-shuo. NO humans NO text. Light only no solid entity in pillar.",
    source: "s1e2-u08",
    sourceVersion: "20260723",
  });
  ...
  const mk = (assetId: string, category: "character" | "scene" | "prop") => ({
    ...
    evidence: [{ kind: "prompt-revision", reference: promptRev.id, note: "s1e2-u08" }],
  });
  // 4+4.5+3.5+3=15
  const panels = [
    {
      title: "G1 金缕点额",
      visualAction: "金光柱边缘一缕金光点向先祖白额先祖俯首应允",
      shotComposition: "中近景",
      filmingMethod: "呼吸感固定",
      startSeconds: 0,
      durationSeconds: 4,
    ...
    {
      title: "G2 第一道纹",
      visualAction: "白额正中金银两丝交缠拧成竖纹第一道流星纹成形",
      shotComposition: "大特写",
      filmingMethod: "微推",
      startSeconds: 4,
      durationSeconds: 4.5,
      promptRevisionId: promptRev.id,
      sourceSpans: [span],
      assets: [mk("char-shuo", "character"), mk("char-dudu", "character")],
    },
    {
      title: "G3 伏姿立约",
      visualAction: "先祖伏下前躯额贴星河新纹在金光里亮着",
      shotComposition: "中景",
      filmingMethod: "呼吸感固定",
      startSeconds: 8.5,
      durationSeconds: 3.5,
    ...
    {
      title: "G4 纹定亮",
      visualAction: "白额新纹特写金银交缠竖纹由灼亮收成温润定住",
  ...
  for (const panel of snap.panels) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const mentions = panel.assets.map((asset, i) => {
        ...
        return {
          id: `u08-m-${panel.index}-${asset.assetId}-${i}-${attempt}`,
          ...
          presence: "required" as const,
          role: `u08-${asset.assetId}-p${panel.index}-${attempt}`,
      ...
      try {
        analysis = await analyzeStudioPanelAssetMentions(ROOT, {
          ...
          mentions,
          resolverVersion: `s1e2-u08-${attempt}`,
        });
        decisions = [];
        for (const proposal of analysis.proposals) {
          ...
          decisions.push(
            await recordStudioMentionDecision(ROOT, {
              receiptId: `u08-dec-p${panel.index}-${proposal.mentionId}-${attempt}`,
      ...
      for (const field of STUDIO_CONTINUITY_FIELDS) {
        ...
        try {
          await appendStudioContinuityObservation(ROOT, {
            operationId: `u08-cont-${panel.id}-${src.assetId}-${field}`,
...
async function main() {
  ...
  if (!packId || !packFingerprint) throw new Error(`freeze missing ${JSON.stringify(freeze).slice(0, 300)}`);
  log(`FREEZE_OK ${packId}`);

  const controlRefs = await resolveControlLocalPaths(packId);
  log(`CONTROL_REFS count=${controlRefs.length}`);

  const runId = `s1e2-u08-ug-grok-${Date.now().toString(36)}`;
  ...
  for (let i = 0; i < 6; i++) {
    try {
      ...
      prepared = await prepareStudioImagegenCall(ROOT, {
        ...
        commandRequestId: `u08-prep-${i}-${Date.now().toString(36)}`,
        expectedRevision: 0,
      });
      break;
    } catch (e: any) {
  ...
  const state = {
    ...
    controlRefs,
    candidateOut: path.join(PROD, "02_candidates/S1E2-U08_4格_A1_CANDIDATE.jpg"),
    reportOut: path.join(PROD, "05_canvas/s1e2-u08-symbiosis-report.json"),
