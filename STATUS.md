...
| earliest_next | A 仍等 owner。B 对照/SSL-5/阅读器/向导/检查器/生成控制/审片/session-snapshot 可查扩写格镜头类型（只解析已有「镜头类型：」行；无包才退锁版可选列；审片只认冻结包；向导不写新冻结行）。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」/「角色/场景/道具/扩写格已完全解决」 |
