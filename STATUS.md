...
| B_earliest | unit-grid brief / 画布检查器已只读露出前镜；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照 + 冻结提示词 + brief/session-snapshot/审片/画布检查器 previousStanding）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 源码已把前镜接到 unit-grid brief 与画布检查器；真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
