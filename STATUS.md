...
| B_earliest | 选区命中可跳对照宫格并滚进视口；真生图验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（per-panel + SSL-5 焦点/前镜 + 选区命中站位 + 跳对照）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 源码选区命中已带前镜并可跳对照；真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
