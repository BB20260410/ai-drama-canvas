...
| B_earliest | 15s 向导已写/露前镜与本格光线/服化；检查器/生成控制/审片/session-snapshot 已露冻结宫格光线服装；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照 + 冻结提示词 + brief/session-snapshot/审片/画布检查器/get_studio_trace + 导演/对照/阅读器追溯 + 15s 向导 previousStanding/光线/服化 + 冻结宫格光线服装只读）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 向导可填光线/服化；检查器/生成控制/审片/session-snapshot 只从冻结包露宫格覆盖。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
