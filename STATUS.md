...
| B_earliest | session-snapshot / 生成控制 / 审片已只读露出冻结前镜；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照前镜 + 冻结提示词 + brief/session-snapshot/审片 previousStanding）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 源码已把前镜接到 session-snapshot / 生成控制 / 审片只读面；真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
