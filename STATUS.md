...
| B_earliest | 冻结提示词已写前镜站位；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照前镜 + 冻结 renderedPrompt 前镜行）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 源码已把前镜写入新冻结提示词；真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
