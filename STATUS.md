...
| ssl5 | `get_studio_script_library_projection` op `ssl5-missing-to-gen-plan`；对照横幅含 focusPanelId / focusPackId / generationPlanDraft / lightingCostumeLine / previousLightingLine / sceneBackReferences / propBackReferences / characterBackReferences / shotTypeLine / styleLockLine / beatLine / earliestCode；recommendedPath 含 create-plan；账本已有对应 plan 则草稿 ready=false，按节点状态写 dispatch/wait/retry/Review；焦点即 earliest 且 earliest 已是 wait/retry/Review/对账时禁止再建议 create-plan/dispatch，对照 `ssl5-earliest-next` 跟 blockedReason；打开对照自动露出焦点宫格；不执行建计划、不 dispatch、不 retry |
