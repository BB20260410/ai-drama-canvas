...
| B_earliest | session-snapshot 无 panelId 已接 unit-grid create-plan 草稿；readiness next 含 create-plan；真生图约束验证仍等 owner |
...
| earliest_next | A 仍等 owner。B session-snapshot 无 panelId 已接 unit-grid create-plan 草稿（只认账本已落盘 pack，不用 readiness 候选；不执行、不 dispatch）。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」/「角色/场景/道具/扩写格/节拍已完全解决」 |
