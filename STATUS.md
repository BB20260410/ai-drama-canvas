...
| B_earliest | 15s 向导已写/露前镜；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照 + 冻结提示词 + brief/session-snapshot/审片/画布检查器/get_studio_trace + 导演/对照/阅读器追溯 + 15s 向导 previousStanding）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 15s 向导已把前镜写进设计/物化 prompt。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
