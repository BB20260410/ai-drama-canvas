...
| 字段 | 当前值 |
|---|---|
| status | `部分完成`。云端 B 轨源码辅助已达极限并停手。整目标未关账 |
...
| B_earliest | earliest / 驾驶舱 / SSL-5 / 对照 / session-snapshot / 生成控制 / pack envelope / readiness / 画布节点 / active-runs / plan / history 信封 next 已按 wait/retry/Review 对齐；15s 向导物化后下一步含 create-plan；session-snapshot / history 信封 / get_studio_trace / 画布检查器 / 阅读器 span 命中 / SSL-5 横幅一致性四态 peek 已接；对照/SSL-5 复用六图闸未放行则禁止再建议 create-plan/dispatch；对照/SSL-5 复用写租约（未持有才插 acquire-lease）+ 对照缺图报告 JSON 可复制；15s 向导复用已加载对照板六图闸/写租约（未加载不自动查）；阅读器复用已算 earliest 露出闸/租约；session-snapshot 写租约只读 peek + 六图闸只读 peek（不暴露 token、不进 fingerprint、不改 nextAction）；画布主按钮/节点 freeze-dispatch 复用已加载 overview 六图闸（未投影不挡；零额外 IPC；不 persist / 不 emit）；执行最近工作流组同样挡六图闸（不 executeWorkflowGroup）；画布主按钮/节点/工作流组复用已加载 unit-grid wait/retry/Review 闸（planned 不挡；零额外 IPC）；阅读器本修订影响复用 P24 script-revision-impact（按需；非预期须人工复核；不猜第一格；不改 getStudioTrace selector）；本修订影响行可点穿对照精确格（无 panelId 只露单元行）；已加载 unexpected 精炼 SSL-5 禁止再建议 create-plan/dispatch（未加载不自动查）；本修订影响可翻下一页（nextCursor 追加去重）+ 对照表/侧栏已加载 unexpected 角标；SSL-5 非预期改去审片；MCP/Core ssl5-missing-to-gen-plan 可传入已取回 revisionImpact（省略不自动查）；create-plan/dispatch 写路径对已取回 unexpected 失败关闭（省略不查；不改 freeze）；readiness/plan 只读 next 对已取回 unexpected 改 Review（省略不查；wait/已 Review 保留；不改 freeze）；向导 Core 物化按规范资产真实 category/name 落盘（缺记录跳过，不再一律 character；与桌面 App 对齐；不改 formatWizardPromptBody）；向导歧义未裁决禁止物化（须显式选用/排除，禁止默认第一候选）；向导露出拆格骨架/建议资产并可改转场·负提示（骨架不进冻结提示词，bond-loop 未关）；Align/Core/App 向导物化校验同源失败关闭；向导建议资产缺规范记录禁止静默跳过（打开露出 category/name，写路径先解析再失败关闭）；**云端源码辅助达极限已停**（不再克隆横幅/hint/校验）；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照 + 冻结提示词 + brief/session-snapshot/审片/画布检查器/get_studio_trace + 导演/对照/阅读器追溯 + 15s 向导 previousStanding/光线/服化/场景回指 + 冻结宫格光线服装只读含 brief/trace + 对照/SSL-5/阅读器/session-snapshot/检查器/生成控制 场景回指 + 检查器/生成控制锁版光线/服化 + SSL-5/阅读器锁版光线服化与回指点穿 + 对照/SSL-5/阅读器/向导/检查器/生成控制/session-snapshot 道具回指 + 对照/SSL-5/阅读器/向导/检查器/生成控制/session-snapshot 角色回指 + 对照/SSL-5/阅读器/向导/检查器/生成控制/审片/session-snapshot 扩写格镜头类型 + 对照/SSL-5/阅读器/向导/检查器/生成控制/审片/session-snapshot 15s 节拍 + unit-grid brief.BEATS 起止秒/扩写格 + 单镜 brief shotTypeLine/beatLine + SSL-5 create-plan 只读草稿 + session-snapshot/生成控制 create-plan 草稿 + 生成控制/pack envelope unit-grid create-plan 草稿 + session-snapshot 无 panelId 整板草稿 / readiness create-plan + 驾驶舱 create-plan nextAction + earliest 单单元 pack/计划 next + 风格锁只读面 + 已有计划则按节点状态写 dispatch/wait/retry/Review + earliest/驾驶舱 nextAction 按节点状态 + SSL-5/对照 earliest wait/retry/Review 禁止再建议 create-plan + session-snapshot/生成控制/pack envelope 单镜草稿跟 unit-grid 在途对齐 + readiness next 跟 unit-grid 在途对齐 + 画布节点下一步跟 unit-grid 在途对齐 + active-runs envelope next 跟本槽/unit-grid 对齐 + 15s 向导物化后下一步含 create-plan + plan 信封 next 跟节点状态对齐 + history 信封 next 跟本页结果对齐 + session-snapshot / history 信封 / get_studio_trace / 画布检查器 / 阅读器 span 命中 / SSL-5 横幅一致性四态 peek + 对照/SSL-5 六图闸 + 对照/SSL-5 写租约 + 对照缺图报告 JSON + 向导复用已加载对照板闸/租约 + 阅读器复用 earliest 闸/租约 + session-snapshot 写租约 peek + session-snapshot 六图闸 peek + 画布主按钮/节点 freeze-dispatch 复用 overview 六图闸 + 执行最近工作流组挡六图闸 + 画布主按钮/节点/工作流组挡 unit-grid 在途 + 阅读器本修订影响 script-revision-impact + impact 点穿对照格 + 已加载 unexpected 精炼 SSL-5 + impact 翻页与对照角标 + MCP 已取回 revisionImpact hint + create-plan/dispatch 写路径 unexpected 失败关闭 + readiness/plan 只读 next unexpected 改 Review + 向导 Core 物化真实资产分类 + 向导歧义未裁决失败关闭 + 向导拆格骨架/建议资产/转场负提示可见 + 向导物化校验同源 + 向导建议资产缺记录失败关闭）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5；**云端 B 源码极限已停** |
| ssl5 | `get_studio_script_library_projection` op `ssl5-missing-to-gen-plan`；对照横幅含 focusPanelId / focusPackId / generationPlanDraft / lightingCostumeLine / previousLightingLine / sceneBackReferences / propBackReferences / characterBackReferences / shotTypeLine / styleLockLine / beatLine / earliestCode / consistencyPeek（复用对照板焦点格/行）/ checkpointLine（复用对照板六图闸）/ writeLeaseLine（复用对照板写租约）；recommendedPath 含 create-plan；账本已有对应 plan 则草稿 ready=false，按节点状态写 dispatch/wait/retry/Review；焦点即 earliest 且 earliest 已是 wait/retry/Review/对账时禁止再建议 create-plan/dispatch；六图闸未放行新槽时同样禁止（earliest wait/retry/Review 文案更具体时保留）；对照板已投影且未持有写租约时路径在 freeze 前插入 acquire-lease（未投影不插；闸/earliest 已挡不插；不改草稿 ready）；对照 `ssl5-earliest-next` / `ssl5-checkpoint-next` / `ssl5-write-lease` 跟 blockedReason/租约行；对照可复制 missingReport JSON；打开对照自动露出焦点宫格；可选 revisionImpact 为调用方已取回的 script-revision-impact 页（焦点 unexpected → recommendedPath=review；省略不自动查；不 import studio-trace）；create-plan/dispatch 写路径对同一 hint 失败关闭（省略不查）；readiness/plan 只读 next 对同一 hint unexpected 改 Review（省略不查）；向导 Core 物化按规范资产真实 category/name 落盘（缺记录跳过，不再一律 character）；向导歧义未裁决禁止物化（须显式选用/排除）；向导露出拆格骨架/建议资产并可改转场·负提示（骨架不进冻结提示词）；Align/Core/App 向导物化校验同源失败关闭；向导建议资产缺规范记录禁止静默跳过；不执行建计划、不 dispatch、不 retry、不执行停检、不抢租约、不暴露 token |
| evidence | `docs/evidence/goal-ssl5-missing-to-gen-mcp-20260826.json`、`goal-script-span-media-map-20260826.json`、`goal-align-consistency-peek-20260826.json`、`goal-reader-span-media-20260826.json`、`goal-episode-unit-media-map-per-panel-20260826.json`、`goal-align-panel-rows-20260826.json`、`goal-ssl5-focus-missing-panel-20260826.json`、`goal-ssl5-reveal-focus-panel-20260826.json`、`goal-library-covered-and-panel-peek-20260826.json`、`goal-align-panel-composition-20260826.json`、`goal-align-panel-standing-handoff-20260826.json`、`goal-reader-span-standing-handoff-20260826.json`、`goal-reader-span-reveal-align-20260826.json`、`goal-align-standing-gaps-20260826.json`、`goal-align-table-panel-select-20260826.json`、`goal-align-row-no-unit-fallback-20260826.json`、`goal-freeze-prompt-previous-standing-20260826.json`、`goal-agent-brief-previous-standing-20260826.json`、`goal-session-snapshot-review-previous-standing-20260826.json`、`goal-unit-grid-brief-canvas-inspector-standing-20260826.json`、`goal-trace-previous-standing-20260826.json`、`goal-director-open-trace-20260826.json`、`goal-align-open-trace-20260826.json`、`goal-wizard-previous-standing-20260826.json`、`goal-wizard-lighting-costume-20260826.json`、`goal-inspector-frozen-lighting-costume-20260826.json`、`goal-session-snapshot-frozen-lighting-costume-20260826.json`、`goal-brief-trace-frozen-lighting-costume-20260826.json`、`goal-align-scene-backrefs-20260826.json`、`goal-session-snapshot-scene-backrefs-20260826.json`、`goal-ssl5-reader-scene-backrefs-20260826.json`、`goal-inspector-lock-lighting-costume-20260826.json`、`goal-inspector-scene-backrefs-20260826.json`、`goal-wizard-scene-backrefs-20260826.json`、`goal-ssl5-reader-lighting-costume-backrefs-20260826.json`、`goal-generation-control-lock-lighting-backrefs-20260826.json`、`goal-prop-backrefs-20260826.json`、`goal-character-backrefs-20260826.json`、`goal-shot-type-extension-20260826.json`、`goal-unit-beat-20260826.json`、`goal-unit-grid-brief-beats-20260826.json`、`goal-panel-brief-shot-type-beat-20260826.json`、`goal-ssl5-generation-plan-draft-20260826.json`、`goal-session-control-plan-draft-20260826.json`、`goal-unit-grid-plan-draft-20260826.json`、`goal-session-unit-grid-plan-draft-20260826.json`、`goal-dashboard-create-plan-next-20260826.json`、`goal-earliest-plan-next-20260826.json`、`goal-style-lock-readonly-20260826.json`、`goal-create-plan-already-exists-dispatch-20260826.json`、`goal-plan-node-status-next-20260826.json`、`goal-earliest-dashboard-plan-status-next-20260826.json`、`goal-ssl5-earliest-blocking-next-20260826.json`、`goal-session-control-unit-grid-blocking-next-20260826.json`、`goal-pack-envelope-unit-grid-blocking-next-20260826.json`、`goal-readiness-unit-grid-blocking-next-20260826.json`、`goal-canvas-node-unit-grid-blocking-next-20260826.json`、`goal-active-runs-unit-grid-blocking-next-20260826.json`、`goal-wizard-post-materialize-create-plan-next-20260826.json`、`goal-plan-envelope-next-20260826.json`、`goal-history-envelope-next-20260826.json`、`goal-session-consistency-peek-20260826.json`、`goal-history-consistency-peek-20260826.json`、`goal-trace-inspector-consistency-peek-20260826.json`、`goal-reader-ssl5-consistency-peek-20260826.json`、`goal-ssl5-checkpoint-gate-20260826.json`、`goal-ssl5-write-lease-missing-report-20260826.json`、`goal-wizard-align-checkpoint-lease-20260826.json`、`goal-reader-checkpoint-lease-20260826.json`、`goal-session-write-lease-20260826.json`、`goal-session-checkpoint-gate-20260826.json`、`goal-canvas-overview-checkpoint-gate-20260826.json`、`goal-canvas-run-workflow-checkpoint-gate-20260826.json`、`goal-canvas-unit-grid-dispatch-gate-20260826.json`、`goal-reader-script-revision-impact-20260826.json`、`goal-reader-impact-align-unexpected-ssl5-20260826.json`、`goal-reader-impact-next-align-mark-20260826.json`、`goal-ssl5-revision-impact-hint-20260826.json`、`goal-write-unexpected-revision-impact-20260826.json`、`goal-readiness-plan-revision-impact-next-20260826.json`、`goal-wizard-suggested-asset-category-20260827.json`、`goal-wizard-unresolved-fail-closed-20260827.json`、`goal-wizard-suggest-clues-visible-20260827.json`、`goal-wizard-validate-homology-20260827.json`、`goal-wizard-missing-asset-fail-closed-20260827.json`、`goal-cloud-b-source-limit-20260827.json` |
| earliest_next | A 仍等 owner。B SSL-5 / 对照 / session-snapshot / 生成控制 / pack envelope / readiness / 画布节点 / active-runs / plan / history 信封 next 已与 wait/retry/Review 对齐；15s 向导物化后下一步含 create-plan；session-snapshot / history 信封 / get_studio_trace / 画布检查器 / 阅读器 span 命中 / SSL-5 横幅一致性四态 peek 只读 LRU（未评估 ≠ 无法检查；不 evaluate；不自动 Review PASS）；对照/SSL-5 复用六图闸未放行则禁止再建议 create-plan/dispatch；对照/SSL-5 复用写租约（未持有才插 acquire-lease）+ 对照缺图报告 JSON 可复制；15s 向导复用已加载对照板六图闸/写租约（未加载不自动查）；阅读器复用已算 earliest 露出闸/租约；session-snapshot 写租约只读 peek + 六图闸只读 peek（不暴露 token、不进 fingerprint、不改 nextAction）；画布主按钮/节点 freeze-dispatch 复用已加载 overview 六图闸（未投影不挡；零额外 IPC）；执行最近工作流组同样挡六图闸；画布主按钮/节点/工作流组复用已加载 unit-grid wait/retry/Review 闸（planned 不挡）；阅读器本修订影响复用 P24 script-revision-impact（按需；非预期须人工复核）；impact 行可点穿对照精确格（无 panelId 只露单元行）；已加载 unexpected 精炼 SSL-5 禁止再建议 create-plan/dispatch（未加载不自动查）；本修订影响可翻下一页（nextCursor 追加去重）+ 对照表/侧栏已加载 unexpected 角标；MCP/Core ssl5-missing-to-gen-plan 可传入已取回 revisionImpact（省略不自动查）；create-plan/dispatch 写路径对已取回 unexpected 失败关闭（省略不查；不改 freeze）；readiness/plan 只读 next 对已取回 unexpected 改 Review（省略不查；不改 freeze）；向导 Core 物化按规范资产真实 category/name 落盘（缺记录跳过，不再一律 character）；向导歧义未裁决禁止物化（须显式选用/排除，禁止默认第一候选）；向导露出拆格骨架/建议资产并可改转场·负提示（骨架不进冻结提示词，bond-loop 未关）；Align/Core/App 向导物化校验同源失败关闭；向导建议资产缺规范记录禁止静默跳过。**云端 B 轨源码辅助达极限已停**；余下 T23 / GUI 探针 / P7 / 真生图 / SSL-6 / bond-loop 仅 owner。禁止写「运行速度已关账」/「100% 契合」/「角色/场景/道具/扩写格/节拍/风格锁已完全解决」 |
...
| models | TokenRouter `--role tr` 仍只打 tokenrouter.com。火山方舟 Agent Plan `/api/plan/v3` 已通。千问 Token Plan 国内 `token-plan.cn-beijing` + `QWEN_TOKEN_PLAN_API_KEY`：`qwen3.8-max-preview` 已 200。完整 Key 不进 git |
| isolation | 未扫正式工程；未写账本；未弹窗；未重做 T23 SQL / N124 CV |
| cloud_env | Draft build `bld-20260826-13a0f906-c113-487d-9131-8b89bbef7d04` 已测。`TOKENROUTER_API_KEY` 按 Personal Runtime Secret 填；后加 Secret 当前 VM 通常看不见，新开 Agent 才注入。无 `env.local` 时不要跑 `tr_chat` |
...
| wave2-projection-readonly-sql | Linux 对真实 `getApprovedTimelineProjection` 手工 sqlite 接线：只 mock inspect；身份/账本/历史走真库。2500 末页 36 只物化 36；`unitTimingQueries=0`。不是 P7 owner。证据 `docs/evidence/runtime-perf-memory-wave2-projection-readonly-sql-20260825.json` |
| wave2-projection-unmocked-inspect | Linux 手工 schema v1 侧车，零 mock 走 `inspectManagedProjectReadOnly` + 真投影。不调用 `createManagedProject` / P7 fixture。2500 末页 36；`unitTimingQueries=0`。不是 P7 owner。证据 `docs/evidence/runtime-perf-memory-wave2-projection-unmocked-inspect-20260825.json` |
...
| wave4-settings | `getCapabilities(projectRoot)` 经 `novel-analysis-provider-settings` 读配置，不 withNovelAnalysisProvider。doctor / 分析工具仍会加载整图。两路审查 PASS |
...
| wave6-reverify-after-readonly | 身份/账本/历史 PASS 只读三刀之后重跑：`typecheck:app` PASS；Linux 41 files / 336 tests PASS；busy ledger 1 条仍需 Darwin dirfd（既有）。两路审查 PASS。未改源码。安装版 T23 / P7 / 列表探针仍 NOT_RUN。证据 `docs/evidence/runtime-perf-memory-wave6-reverify-after-readonly-20260825.json` |
| wave6-reverify-after-unmocked-inspect | 零 mock inspect 之后重跑：`typecheck:app` PASS；Linux 41 files / 322 tests PASS；busy ledger 1 条仍需 Darwin dirfd（既有）。两路审查 PASS。未改源码。安装版 T23 / P7 / 列表探针仍 NOT_RUN。证据 `docs/evidence/runtime-perf-memory-wave6-reverify-after-unmocked-inspect-20260825.json` |
| wave6-darwin-ci | macos-latest CI check **success** on `0b46327`（run `32881922045`）、源码 `e1ea3f1`（run `32889586061`）、扩大复验 `36abafb`（run `32891408457`）、真 sqlite 接线 `43945a2`（run `32906441251`，5 tests）、零 mock inspect `2d8ce47`（run `32906906873`，6+5 tests）与 HEAD 复验 `4b1824a`（run `32907435248`，320 files）。不是安装版 T23 |
| wave7-runtime-hotpath | `getStudioMedia` 只读旁路 + 同 SHA 单飞；derivative `withProtocolDatabase` 单次开库；核心裁剪 rebuild 并入 rAF。二次 `assertDatabaseFiles` 与 worker 增量 schedule 保留。`typecheck:app` PASS；Linux 7 files / 139 tests PASS。不是 P7 / T23 / GUI。证据 `docs/evidence/runtime-perf-memory-wave7-runtime-hotpath-20260826.json` |
| external-review | 2026-08-26 分席实调：千问主审 **无 must-fix**。已按主审执行：不拆 ledger、不全量异步化 rebuildGraph。入口 `python3 scripts/external-seat-review.py --brief <简报> -o /tmp/seat-review`（`--seats` / `--with-doubao`）。源码合同 2 files / 4 tests PASS；`--seats qwen` 短探测回复「千问已通」。证据 `docs/evidence/runtime-perf-memory-external-review-20260826.json`、`runtime-perf-memory-external-seat-runner-20260826.json`。整计划仍部分完成 |
...
| earliest_next | **等待 owner**。§1.3 三刀 + 身份/账本/历史只读 + Linux 真投影 sqlite 接线 + 零 mock inspect + Wave 7 画布媒体热路径已落地；Darwin CI `43945a2`/`2d8ce47`/`4b1824a` 已绿。仅用户能解：①安装版 T23 ②列表翻页 GUI 探针 `aicanvas-studio://media/* = 0` ③P7 owner 有界对照。禁拆 command-bus ledger。禁止半吊子改 checkpoint 写初始化。禁止 N124 / 禁止写「运行速度已关账」 |
