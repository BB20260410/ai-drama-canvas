...
| 字段 | 当前值 |
|---|---|
| status | `阻塞`（§1.3 by-id / 诊断懒展开 / lean 身份 + 身份只读打开 + 时间线账本批读只读旁路 + 历史 PASS 活库只读 + Linux 真投影 sqlite 接线 + 零 mock inspect 已落地；整计划仍 `部分完成`）。§1 八条 1/2/4/7/8 已完成，3/5/6 部分完成。等待 owner / Darwin。禁止写「运行速度已关账」 |
...
| earliest_next | **等待 owner**。§1.3 三刀 + 身份/账本/历史只读 + Linux 真投影 sqlite 接线 + 零 mock inspect 已落地。仅用户能解：①安装版 T23 ②列表翻页 GUI 探针 `aicanvas-studio://media/* = 0` ③P7 owner 有界对照。禁拆 command-bus ledger。禁止半吊子改 checkpoint 写初始化。禁止 N124 / 禁止写「运行速度已关账」 |
