...
| B_earliest | SSL-5 / 阅读器已露出锁版光线服化，场景回指可点穿精确宫格；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照 + 冻结提示词 + brief/session-snapshot/审片/画布检查器/get_studio_trace + 导演/对照/阅读器追溯 + 15s 向导 previousStanding/光线/服化/场景回指 + 冻结宫格光线服装只读含 brief/trace + 对照/SSL-5/阅读器/session-snapshot/检查器 场景回指 + 检查器锁版光线/服化 + SSL-5/阅读器锁版光线服化与回指点穿）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B SSL-5/阅读器/向导/检查器/对照可查锁版光线服化与更早同场景宫格（快照提及 ≠ BindingSet；向导不自动加载对照板；阅读器仅用户点穿回指才加载对照板）。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
