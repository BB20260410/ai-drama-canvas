...
| models | TokenRouter CLI 已装到 `~/.grok/bin`（仅 `--role tr`）。Cloud 无 `env.local` 时只用 `novel_chat.py --role tr`（读 Runtime Secret / 环境变量），不要跑 `tr_chat`。完整 Key 不进 git。500 / `do_request_failed` 重试 |
