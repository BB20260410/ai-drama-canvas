...
| 字段 | 当前值 |
|---|---|
| status | `阻塞`（§1.3 by-id 残差已落地；其后本 Cloud 可执行项已尽；整计划仍 `部分完成`）。§1 八条 1/2/4/7/8 已完成，3/5/6 部分完成。等待 owner / Darwin。禁止写「运行速度已关账」 |
...
| earliest_next | **等待 owner**。§1.3 by-id 残差已落地；其后本 Cloud 无下一步可执行项。仅用户能解：①安装版 T23 ②列表翻页 GUI 探针 `aicanvas-studio://media/* = 0` ③P7 owner 有界对照。禁拆 command-bus ledger。禁止 N124 / 禁止写「运行速度已关账」 |
