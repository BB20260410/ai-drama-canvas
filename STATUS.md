...
| B_earliest | 对照/SSL-5/阅读器/向导/检查器/生成控制/session-snapshot 已露出跨单元道具回指；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照 + 冻结提示词 + brief/session-snapshot/审片/画布检查器/get_studio_trace + 导演/对照/阅读器追溯 + 15s 向导 previousStanding/光线/服化/场景回指 + 冻结宫格光线服装只读含 brief/trace + 对照/SSL-5/阅读器/session-snapshot/检查器/生成控制 场景回指 + 检查器/生成控制锁版光线/服化 + SSL-5/阅读器锁版光线服化与回指点穿 + 对照/SSL-5/阅读器/向导/检查器/生成控制/session-snapshot 道具回指）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
| ssl5 | `get_studio_script_library_projection` op `ssl5-missing-to-gen-plan`；对照横幅含 focusPanelId / lightingCostumeLine / previousLightingLine / sceneBackReferences / propBackReferences；打开对照自动露出焦点宫格；不 dispatch |
...
| earliest_next | A 仍等 owner。B 对照/SSL-5/阅读器/向导/检查器/生成控制/session-snapshot 可查更早同场景/同道具宫格（快照提及 ≠ BindingSet；有冻结包仍只认覆盖行；向导不自动加载对照板）。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」/「场景/道具已完全解决」 |
