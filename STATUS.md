...
| B_earliest | 阅读器选区命中已露前镜/构图/运镜；真生图验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（per-panel + SSL-5 焦点/前镜 + 选区命中站位只读）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 源码选区命中已带前镜；真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
