...
| earliest_next | A 仍等 owner。B 对照/SSL-5/阅读器/向导/检查器/生成控制/审片/session-snapshot 可查 15s 节拍（只读冻结包 target 起止秒；无包检查器/生成控制用驾驶舱已有 start/end/duration；审片只认冻结包；向导不写新冻结行）。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」/「角色/场景/道具/扩写格/节拍已完全解决」 |
