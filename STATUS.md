...
| models | TokenRouter `--role tr` 仍只打 tokenrouter.com。火山方舟走 Agent Plan `/api/plan/v3` + `ARK_API_KEY`（`~/.grok/env.local`）；`doubao-seed-evolving` / `glm-5.3` 已 200。普通 `/api/v3` 401。完整 Key 不进 git |
