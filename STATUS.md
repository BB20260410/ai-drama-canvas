...
| B_earliest | 15s 向导已写/露前镜与本格光线/服化；检查器/生成控制/审片已露冻结宫格光线服装；真生图约束验证仍等 owner |
...
| earliest_next | A 仍等 owner。B 向导可填光线/服化；检查器/生成控制/审片只从冻结包露宫格覆盖。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
