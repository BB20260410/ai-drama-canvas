...
| earliest_next | A 仍等 owner。B SSL-5 / 对照 / session-snapshot / 生成控制 / pack envelope / readiness / 画布节点 / active-runs next 已与 earliest/unit-grid wait/retry/Review 对齐；15s 向导物化后下一步含 create-plan（不执行、不派发、不重试）。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」/「角色/场景/道具/扩写格/节拍/风格锁已完全解决」 |
