...
| B_earliest | 等待 owner 真生图验证前镜交接；对照侧栏已露运镜/光线/前镜/快照提及 |
| dual_track_gate | A_done=no；B_demo=部分（per-panel + SSL-5 焦点宫格/前镜 + 构图站位只读）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 源码前镜交接已挂，真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
