...
| earliest_next | A 仍等 owner。B SSL-5 / 对照已与 earliest wait/retry/Review 对齐（不执行、不派发、不重试）。session-snapshot / 生成控制单镜草稿与驾驶舱 nextAction 仍可能分叉。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」/「角色/场景/道具/扩写格/节拍/风格锁已完全解决」 |
