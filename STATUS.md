...
| B_earliest | 单镜 Agent brief 已暴露前镜站位；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照前镜 + 冻结提示词 + brief.previousStanding）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B 源码已把前镜写入冻结提示词与 Agent brief；真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
