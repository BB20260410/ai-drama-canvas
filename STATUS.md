...
| B_earliest | session-snapshot 已可查跨单元场景回指（只读 SQL 快照提及）；真生图约束验证仍等 owner |
| dual_track_gate | A_done=no；B_demo=部分（对照 + 冻结提示词 + brief/session-snapshot/审片/画布检查器/get_studio_trace + 导演/对照/阅读器追溯 + 15s 向导 previousStanding/光线/服化 + 冻结宫格光线服装只读含 brief/trace + 对照场景回指 + session-snapshot 跨单元场景回指）；SSL≥ssl3 源码有、ssl5 计划已暴露；fit_band≠L5 |
...
| earliest_next | A 仍等 owner。B session-snapshot 可查更早同场景宫格（只读 SQL 快照提及 ≠ BindingSet）。真实验证需正式工程。禁止写「运行速度已关账」/「100% 契合」 |
