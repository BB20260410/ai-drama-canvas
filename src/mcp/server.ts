...
const voiceIdentityInputShape = { name: z.string().min(1).max(160), provider: z.string().max(120).optional(), providerVoiceId: z.string().max(500).optional(), language: z.string().max(80).optional(), description: z.string().max(20_000).optional(), samplePaths: z.array(z.string()).max(100).optional(), characterItemIds: z.array(z.string()).max(500).optional(), hardLockId: z.string().min(1).optional(), tags: z.array(z.string()).max(100).optional() };
...
registrar.registerTool(
  ...
  {
    title: "执行幂等写命令",
    description: "Codex/Grok 首选写入口。每次携带 requestId 和 idempotencyKey；相同参数重试返回原结果，不同参数复用同一键会拒绝。生产 require 模式：生图相关写必须先 acquire_studio_project_write_lease 并传 writeLeaseHolderId+writeLeaseToken（无租约不准写）。generation_unknown 禁止 re-dispatch。scan_project 支持进度通知。",
    inputSchema: {
      projectRoot: projectRootSchema,
