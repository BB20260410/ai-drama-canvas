 ...
 *
 * 只读保证：只读打开已有账本（readOnly + query_only），不建库、不迁移、不复制整库；
 * 表/库不存在一律返回空结果（老库防御）。时间线走 ReadOnly 旁路，跳过二次 inspect。
 *
 * 闭合核验口径（如实声明，不假验）：
...
export const HISTORICAL_FACTS_LEDGER_NOT_REGULAR_FILE_MESSAGE = "generation ledger 数据库必须是无符号链接的普通文件。";
...
export interface StudioGenerationProjectionSelectionFactsReadContext {
  databasePath: string;
  projectRoot: string;
  mediaCasRoot: string;
}
...
function emptySelectionFacts(): StudioGenerationProjectionSelectionFacts {
  return {
...
/** 时间线历史事实专用：只读打开已有账本。不 ensure、不建库、不迁移、不复制整库。 */
function openHistoricalFactsReadOnly(databasePath: string): DatabaseSync {
  const metadata = lstatSync(databasePath);
  if (metadata.isSymbolicLink() || !metadata.isFile()) {
    throw new Error(HISTORICAL_FACTS_LEDGER_NOT_REGULAR_FILE_MESSAGE);
  }
...
function queryHistoricalFactsFromOpenDatabase(
  db: DatabaseSync,
  input: {
  ...
  latestImportByUnit: Map<string, HistoricalImportJoinRow>;
  packFingerprintByRunId: Map<string, string>;
} {
  ...
  return { latestImportByUnit, packFingerprintByRunId };
}

async function verifyHistoricalPassCandidates(
  latestImportByUnit: Map<string, HistoricalImportJoinRow>,
  input: { units: Array<{ unitId: string; revision: number }> },
  context: StudioGenerationProjectionSelectionFactsReadContext,
): Promise<Map<string, StudioHistoricalPassCandidate>> {
  const revisionByUnit = new Map(input.units.map((unit) => [unit.unitId, unit.revision]));
  const mediaCasRoot = context.mediaCasRoot;
  ...
  for (const [unitId, row] of latestImportByUnit) {
    ...
    if (row.pack_content_relpath) {
      ...
      const projectRootResolved = path.resolve(context.projectRoot);
...
export async function readStudioGenerationProjectionSelectionFactsReadOnly(
  ...
  if (input.units.length === 0 && input.generationRunIds.length === 0) return emptySelectionFacts();
  // 账本尚未创建（无冻结/派发历史的工程）：空事实，非错误。
  if (!existsSync(context.databasePath)) return emptySelectionFacts();

  const db = openHistoricalFactsReadOnly(context.databasePath);
  let readTransactionOpen = false;
  let latestImportByUnit = new Map<string, HistoricalImportJoinRow>();
  let packFingerprintByRunId = new Map<string, string>();
  try {
    db.exec("BEGIN");
    readTransactionOpen = true;
    const queried = queryHistoricalFactsFromOpenDatabase(db, input);
    latestImportByUnit = queried.latestImportByUnit;
    packFingerprintByRunId = queried.packFingerprintByRunId;
    db.exec("COMMIT");
  ...
  const historicalPassByUnit = await verifyHistoricalPassCandidates(latestImportByUnit, input, context);
 ...
 * 不静默退化为"无历史 PASS"的假相。
 * s1e1 / 诊断入口：inspect 后转 ReadOnly。时间线投影直接走 ReadOnly，避免二次 inspect。
 */
export async function readStudioGenerationProjectionSelectionFacts(
  ...
  return readStudioGenerationProjectionSelectionFactsReadOnly({
