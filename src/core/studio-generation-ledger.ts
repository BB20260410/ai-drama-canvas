...
interface DetachedUnknownObservationRow {
  sequence: number;
...
interface DetachedUnknownDispositionRow {
  sequence: number;
...
function serializePack(pack: AnyStudioGenerationFreezePack): Buffer {
  return Buffer.from(`${JSON.stringify(stableValue(pack), null, 2)}\n`, "utf8");
}
...
async function managedLedgerPaths(projectRoot: string): Promise<{ paths: LedgerPaths; projectId: string }> {
...
function openDatabase(paths: LedgerPaths): DatabaseSync {
...
function detachedUnknownSemantic(input: {
  targetKey: string;
  unitId: string;
  unitRevision: number;
  unitFingerprint: string;
...
function detachedUnknownRecord(row: DetachedUnknownObservationRow): StudioDetachedGenerationUnknownObservation {
...
function detachedUnknownDispositionSemantic(input: {
  observation: StudioDetachedGenerationUnknownObservation;
  authorizationEvidenceReference: string;
  authorizationTextSha256: string;
  reason: string;
  projectContext: StudioDetachedGenerationUnknownDisposition["projectContext"];
}) {
  return {
    schemaVersion: 1 as const,
...
function detachedUnknownDispositionRecord(
  row: DetachedUnknownDispositionRow,
  observation: StudioDetachedGenerationUnknownObservation,
  idempotentReplay: boolean,
): StudioDetachedGenerationUnknownDisposition {
...
async function inspectStudioImagegenQuarantineEvidence(input: {
  ...
  let candidateInfo: sharp.OutputInfo;
  try {
