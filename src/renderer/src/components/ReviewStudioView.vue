      ...
      <aside class="review-queue">
        ...
        <div class="queue-list">
          <button v-for="entry in queue" :key="entry.item.id" type="button" :class="{ active: entry.item.id === activeId }" :data-item-id="entry.item.id" @click="select(entry.item.id)">
            <figure><img v-if="entry.item.thumbnailPath" loading="lazy" decoding="async" :src="assetUrl(entry.item.thumbnailPath)" :alt="`${entry.item.title} 验收缩略图`" /><span v-else><ScanEye :size="17" /></span></figure>
            ...
            <article v-for="(artifact, index) in displayedArtifacts" :key="artifact.id" class="media-pane">
              ...
              <div class="media-frame">
                <div class="media-canvas" :style="mediaCanvasStyle(artifact)">
                  ...
                  <img v-else :src="reviewAssetUrl(artifact)" :alt="artifactLabel(artifact)" decoding="async" />
              ...
              <footer><div><span>{{ artifact.kind }} · {{ artifact.variant }}</span><small>{{ mediaFacts(artifact) }}</small><p>{{ artifact.path }}</p></div><div><button type="button" @click="reveal(artifact.path)"><FolderOpen :size="13" /> 文件</button><button v-if="!artifact.authoritative && !artifact.deprecated && artifact.check.ok" type="button" data-testid="review-set-authority" :disabled="settingAuthority" :title="settingAuthority ? '正在处理，不能再设为权威' : undefined" @click="setAuthority(artifact)"><Crown :size="13" /> 设为权威</button></div></footer>
            </article>
          </section>
        </template>
      </main>
        ...
        <template v-if="active">
          ...
          <div class="criteria-list">
            <article v-for="criterion in criteria" :key="criterion.key" :class="`criterion-${criterion.result || 'empty'}`">
              ...
              <div class="criterion-actions"><button type="button" :aria-label="`${criterionLabels[criterion.key]}：通过`" :class="{ active: criterion.result === 'pass' }" @click="criterion.result = 'pass'"><Check :size="12" /></button><button type="button" :aria-label="`${criterionLabels[criterion.key]}：不通过`" :class="{ active: criterion.result === 'fail' }" @click="criterion.result = 'fail'"><X :size="12" /></button><button type="button" :class="{ active: criterion.result === 'na' }" @click="criterion.result = 'na'">N/A</button></div>
          ...
          <div class="decision-actions"><button type="button" data-testid="review-submit-pending" :disabled="submitting || !annotationsValid" :title="submitting ? '正在处理，不能再提交验收' : undefined" @click="submit('pending')">待定</button><button type="button" class="rework" data-testid="review-submit-rework" :disabled="submitting || !canRework" :title="submitting ? '正在处理，不能再提交验收' : undefined" @click="submit('rework')">判定返工</button><button type="button" class="pass" data-testid="review-submit-pass" :disabled="submitting || !canPass" :title="submitting ? '正在处理，不能再提交验收' : undefined" @click="submit('pass')"><CheckCircle2 :size="14" /> 视觉通过</button></div>
          <section class="review-history"><div><span>验收历史</span><b>{{ history.length }}</b></div><article v-for="record in history" :key="record.id"><i :class="decisionClass(record.decision)"></i><div><b>{{ decisionLabel(record.decision) }} · {{ record.resultingStatus }}</b><span>{{ formatTime(record.createdAt) }} · {{ record.reviewer }}</span><p v-if="record.note">{{ record.note }}</p><ul v-if="record.visualConstraintAttestations?.length"><li v-for="attestation in record.visualConstraintAttestations" :key="`${attestation.panelId}:${attestation.ruleId}`"><b>{{ attestation.result === "pass" ? "P3 人工核验通过" : "P3 人工核验失败" }}</b><span>{{ attestation.panelId }} · {{ attestation.ruleId }}</span><p v-if="attestation.note">{{ attestation.note }}</p></li></ul><ul v-if="record.annotations?.length"><li v-for="annotation in record.annotations" :key="annotation.id"><b>{{ annotationTypeLabels[annotation.type] }}</b><span>{{ artifactName(annotation.artifactId) }}<template v-if="annotation.timeSeconds !== undefined"> · {{ annotationTime(annotation.timeSeconds) }}</template></span><p>{{ annotation.text }}</p></li></ul></div></article><p v-if="historyError" class="history-error" data-testid="review-history-error" role="alert">验收历史读取失败：{{ historyError }}</p><p v-if="!historyError && !history.length" class="no-history">尚无视觉验收记录</p></section>
        </template>
      </aside>
    </div>
  </section>
...
const settingAuthority = ref(false);
let loadRevision = 0;
const historyRequestGate = createProjectScopedActionGate();
...
onBeforeUnmount(() => {
  loadRevision += 1;
  historyRequestGate.dispose();
  reviewVideoElements.clear();
});
...
async function resetActive() {
  ...
  if (!entry) return;
  const historyScope = historyRequestGate.begin(projectRoot, itemId);
  let nextHistory: ReviewRecord[];
  try {
    nextHistory = await window.canvasApi.listReviewRecords(projectRoot, { itemId, limit: 50 });
  } catch (error) {
    if (!historyRequestGate.isCurrent(historyScope, props.projectRoot, activeId.value)) return;
    historyError.value = message(error);
    emit("failed", historyError.value);
    return;
  }
  if (historyRequestGate.isCurrent(historyScope, props.projectRoot, activeId.value)) {
...
async function setAuthority(artifact: Artifact) { if (!active.value || settingAuthority.value) return; settingAuthority.value = true; try { await window.canvasApi.setAuthoritativeArtifact(props.projectRoot, active.value.item.id, artifact.id, "在导演验收台选择权威版本"); await load(); emit("updated", "权威版本已更新，验收内容快照已刷新"); } catch(error){ emit("failed",message(error)); } finally { settingAuthority.value = false; } }
async function submit(decision: ReviewDecision) {
  if (!active.value || !artifactA.value || submitting.value) return;
...
.queue-list button{content-visibility:auto;contain-intrinsic-size:auto 86px}
</style>
