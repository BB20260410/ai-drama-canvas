...
const props = defineProps<{
  ...
  panelStyleLockLine?: string | null;
  panelBeatLine?: string | null;
  panelConsistencyPeekLine?: string | null;
...
const nodeActionNextLine = computed(() =>
  props.nodeActionPanel?.fields.find((field) => field.key === "next")?.value ?? "",
);
...
.node-action-panel header { display: flex; justify-content: space-between; gap: 8px; flex-wrap: wrap; margin-bottom: 8px; font-size: 12px; }
.node-action-panel header [data-testid="managed-canvas-inspector-next"] { color: var(--msc-text-2); font-size: 11px; }
