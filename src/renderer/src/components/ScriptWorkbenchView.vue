  ...
  <section class="script-workbench">
    <aside class="document-rail">
      ...
      <div v-if="creating" class="create-document">
        ...
        <button class="primary-button" type="button" data-testid="script-create-document" :disabled="createBusy || saving" :title="(createBusy || saving) ? '正在处理，不能再创建文档' : undefined" @click="createDocument">{{ createBusy ? "创建中" : "创建 Markdown" }}</button>
      </div>
      ...
      <template v-if="activeDocument">
        <header class="editor-header">
          ...
          <div class="editor-actions">
            ...
            <button class="primary-button" type="button" data-testid="script-save-document" :disabled="saving || createBusy || !dirty" :title="(saving || createBusy) ? '正在处理，不能再保存' : undefined" @click="save"><Save :size="15" /> {{ saving ? "保存中" : "保存版本" }}</button>
          </div>
        </header>
